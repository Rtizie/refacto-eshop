...
def handle_cart():
	...
	for item in session['cart']:
		if 'cart' not in session or session['cart'] is []:
			return [],0,0
		...
		products.append({'id': shirt.id, 'name': shirt.name,'color': item['color'],'size': item['size'],'quantity': quantity, 'total': total, 'index': index})
		index += 1
